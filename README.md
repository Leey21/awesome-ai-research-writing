...
下列为可通过 MCP 协议接入的远程服务（与上文基于 `SKILL.md` 的 OpenSkills 技能包不同，需在 Cursor 等环境中单独配置 MCP 服务器）。

- **[Helium MCP](https://github.com/connerlambden/helium-mcp)** — [远程 MCP 端点](https://heliumtrades.com/mcp)。提供市场情报与期权定价相关工具，以及多来源新闻检索与视角对比类工具，便于写作 Related Work 或背景段落时核对公开信息与报道差异。
