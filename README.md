...
### Part III: 本地接入Overleaf进行Vibe Writing
- [如何在Overleaf中进行Vibe Writing](#连接你的Overleaf进行Vibe-Writing)
...
# Part III: 本地接入 Overleaf 进行 Vibe Writing

## 连接你的Overleaf进行Vibe Writing
> 🎯 **适用对象**：本部分内容主要面向经常使用 Cursor、Claude Code 等 AI coding 工具并且惯用 Overleaf 进行论文写作的用户
>
> 💡 **使用说明**：通过 Overleaf Workshop 插件在本地连接 Overleaf，配合前文配置的 Skills，让 AI 助手直接理解并操作全文内容

---

## 配置步骤

### 步骤 1：安装 Overleaf Workshop 插件

1. 在 VSCode 插件市场搜索并安装 [Overleaf Workshop](https://marketplace.visualstudio.com/items?itemName=iamhyc.overleaf-workshop)
2. 在左侧栏选择 `Login with Cookies`，从浏览器获取 Cookies 进行登录
3. 右键选择需要连接的 Project，点击 `Open Project Locally` 并选择本地同步文件夹

详细配置方法可参考 [Overleaf Workshop 官方文档](https://github.com/overleaf-workshop/Overleaf-Workshop)

![Overleaf 本地连接配置示意](images/overleaf_example.png)

### 步骤 2：配置本地 LaTeX 编译环境

**安装 LaTeX 发行版**
- **macOS**: `brew install --cask mactex`
- **Linux**: `sudo apt install texlive-full`
- **Windows**: 下载并安装 [MiKTeX](https://miktex.org/download) 或 TeX Live
...
配置完成后，在 Cursor、Claude Code 等工具中配合前文介绍的 Skills 使用：

- AI 可读取完整论文内容，理解全文上下文
- 直接对话即可修改 .tex 文件，无需复制粘贴
- 结合 `20-ml-paper-writing`、`humanizer` 等 Skills 实现高效写作

---

## ⚠️ 注意事项

**避免同步冲突**：不要同时在网页版 Overleaf 和本地 VSCode 中编辑同一项目，容易造成内容丢失。建议写作时仅使用本地，在线协作时仅使用网页版。
