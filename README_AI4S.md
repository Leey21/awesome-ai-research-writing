...
如果继续使用通用的 CS Prompt，你的论文会充斥着“AI 味”极重的工程术语，从而掩盖了科学发现的光芒，导致跨学科沟通失败。

---

# 修改部分

> AI4S 的核心在于**“用 AI 工具解决科学问题”**，因此 Prompt 的角色设定（Role）和约束条件（Constraints）需要从“CS 会议审稿人”转变为“科学期刊编辑/跨学科专家”。

## 1. 表达润色

**修改逻辑**：此 Prompt 强化了**Storytelling（故事性）**和**Scientific Impact（科学影响力）**。
...
## 2. 论文架构图/机理图（Schematic Design）

**修改逻辑**：CS 会议图通常是方块和箭头的堆砌（Vector style）。但 AI4S 论文需要**“机理示意图” (Schematic)**，通常结合 3D 渲染（如蛋白质、分子、流体）与 AI 数据流。

````markdown
# Role
你是一位世界顶尖的科学插画师（Scientific Illustrator），类似于 BioRender 的首席设计师或 Nature 期刊的御用绘图师。你擅长将复杂的“AI + Science”概念转化为直观、美观且科学准确的机制图（Schematic）。

# Task
请阅读我提供的【方法/原理描述】，设计一张用于顶级期刊（Nature/Science）正文第一页的**概念原理图（Conceptual Figure 1）**。
...
**修改逻辑**：AI4S 更看重**物理约束（Physical Consistency）**和**统计显著性**。
...
# Task
请分析我提供的【实验数据】，撰写一段符合 PNAS/Nature Communications 标准的结果分析（Results）。
